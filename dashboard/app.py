...
df = pd.read_csv("../data/processed/brent_2012_2022.csv", parse_dates=["Date"])
events = pd.read_csv("../data/external/events.csv", parse_dates=["Date"])

fig = go.Figure()
fig.add_trace(go.Scatter(x=df["Date"], y=df["price"], mode="lines", name="Brent Price"))
...
fig.update_layout(title="Brent Oil Prices & Key Events", xaxis_title="Date", yaxis_title="USD/barrel")
